...
export const calculatedTotalDayEarned = (day: any) => {
   const RATES = { CONNECT_TV: 150, CONNECT_NO_TV: 100, ADDON_PON: 100, ADDON_ETH: 75, RECONNECT: 80, EXTRA_HOUR: 100, DUTY_HOUR: 120, BROUGHT_CLIENT: 150, CONNECT_UO: 180 };
    return ((day?.connect_tv || 0) * RATES.CONNECT_TV) + ((day?.connect_no_tv || 0) * RATES.CONNECT_NO_TV) + ((day?.addon_pon || 0) * RATES.ADDON_PON) + ((day?.addon_eth || 0) * RATES.ADDON_ETH) + ((day?.reconnect || 0) * RATES.RECONNECT) + ((day?.extra_hours || 0) * RATES.EXTRA_HOUR) + ((day?.duty || 0) * RATES.DUTY_HOUR) + ((day?.brought_clients || 0) * RATES.BROUGHT_CLIENT) + ((day?.connect_uo || 0) * RATES.CONNECT_UO);
}
            ...
            return (
              <div key={day.date} className="bg-slate-900/50 p-3 rounded-xl border border-slate-700/30 text-[11px] space-y-1.5">
                <div className="flex justify-between items-center border-b border-slate-800/60 pb-1">
                  <span className="font-extrabold text-slate-200">📅 {onFormat(day.date)}</span>
