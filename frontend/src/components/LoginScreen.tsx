...
export function LoginScreen({ onSubmit, email, setEmail, pass, setPass }: LoginProps) {
  return (
    <div className="w-full max-w-md mt-16 flex flex-col space-y-6 animate-[fadeIn_0.2s_ease-out]">
      ...
      <header className="flex flex-col items-center text-center space-y-2.5">
        ...
        <p className="text-slate-400 text-xs font-medium pt-0.5">Вхід у твій персональний фінансовий кабінет</p>
      </header>
      ...
      <div className="bg-slate-800/60 p-6 rounded-2xl border border-slate-700/50 shadow-2xl">
        <form onSubmit={onSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide flex items-center gap-1.5">
              <i className="fa-solid fa-envelope text-slate-500"></i> Електронна пошта
            </label>
          ...
          <div className="space-y-1.5">
            ...
            <input type="password" placeholder="••••••••" value={pass} onChange={(e) => setPass(e.target.value)} className="w-full p-3 bg-slate-900/60 border border-slate-700 rounded-xl text-white focus:outline-none focus:border-emerald-500 text-xs font-semibold" required />
          </div>
        ...
        <div className="text-center border-t border-slate-700/50 pt-4 mt-4">
          <Link to="/register" className="text-xs text-emerald-400 hover:text-emerald-300 font-bold transition-all">
