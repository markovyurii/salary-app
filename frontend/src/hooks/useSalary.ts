...
export function useSalary() {
  ...
  const [userToken, setUserToken] = useState<string | null>(() =>
    localStorage.getItem('salary_app_token'),
  );
  ...
  const [workLog, setWorkLog] = useState<any>({
    connect_with_tv: 0,
    connect_with_tv_shared: 0,
    connect_internet_solo: 0,
    connect_internet_shared: 0,
    connect_tv_solo: 0,
    connect_tv_shared: 0,
    reconnect: 0,
    connect_uo: 0,
  ...
  const fetchSalaryFromBackend = async () => {
    ...
    try {
      const response = await fetch(
        `https://salary-backend-woq5.onrender.com/api/salary?month=${selectedMonth}&year=${selectedYear}`,
        {
      ...
      if (data.calculations?.car_amortization)
        setAmortizationInput(data.calculations.car_amortization.toString());
      if (data.calculations?.bonus_percent !== undefined)
        setBonusInput(data.calculations.bonus_percent.toString());
  ...
  const fetchHistoryFromBackend = async () => {
    ...
    try {
      const response = await fetch(
        `https://salary-backend-woq5.onrender.com/api/work-log/?month=${selectedMonth}&year=${selectedYear}`,
        {
          headers: { Authorization: `Bearer ${userToken}` },
        },
      );
  ...
  const updateBaseSalaryInDb = async (customPayload?: any) => {
    ...
    try {
      const response = await fetch(
        `https://salary-backend-woq5.onrender.com/api/profile/update`,
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) throw new Error('Failed');
      if (bodyPayload.base_salary !== undefined)
        setSalaryInput(bodyPayload.base_salary.toString());
      if (bodyPayload.car_amortization !== undefined)
        setAmortizationInput(bodyPayload.car_amortization.toString());
      if (bodyPayload.is_driver !== undefined)
        setIsDriver(bodyPayload.is_driver);
  ...
  const handleAuthAction = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegistering) {
        ...
        if (authData?.user) {
          await supabaseAuthClient.from('profiles').insert({
            id: authData.user.id,
            full_name: authName,
            base_salary: 19200,
          });
  ...
  const saveCardPayment = async () => {
    if (!userToken || !cardPaymentInput || Number(cardPaymentInput) <= 0)
